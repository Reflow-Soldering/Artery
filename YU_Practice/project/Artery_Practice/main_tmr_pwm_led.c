...
// 현재까지의 논의를 반영하여 Current_PWM_Value를 uint32_t로 변경
uint32_t Current_PWM_Value = 0; // 시작은 가장 밝게 (다운카운트 Mode A)

// PWM_Period를 실제 타이머의 주기(ARR)로 사용할 충분히 큰 값으로 설정
// 이전 논의에서 0x64(100)로의 변경을 시도했으므로, 100을 기준으로 합니다.
// 필요에 따라 0xFF(255)나 0x3FF(1023) 등으로 더 크게 설정할 수 있습니다.
uint32_t PWM_Period = 0x64; // ARR 값: 100 (십진수)
...
int main(void)
{
  ...
  /* initialize key */
    ...
    //timer3 초기화
    // tmr_base_init(타이머, 주기(ARR), 프리스케일러(PSC))
    // PSC는 0x200 (512)으로 유지
    tmr_base_init(TMR3, PWM_Period, 0x200); // ARR: PWM_Period (100), PSC: 512
    ...
    int direction = 1; // int로 변경된 것은 올바름


    while(1)
  {
        // 3에서 문제가 발생한다고 하셨으므로, 일단 5를 유지합니다.
        // 필요하다면 1, 2 등으로 테스트해보세요.
        Current_PWM_Value += direction * 5; // Adjust '5' to change fading speed/smoothness
    ...
    delay_ms(100); // Adjust '50' for desired fade speed
