...
/** @addtogroup 437_ADC_vbat_monitor ADC_vbat_monitor
  * @{
  */
...
static void adc_config(void)
{
  ...
  adc_ordinary_channel_set(ADC1, ADC_CHANNEL_18, 1, ADC_SAMPLETIME_47_5);
...
int main(void)
{
  ...
  while(1)
  {
    ...
    /* adc voltage battery disable */
    adc_voltage_battery_enable(FALSE);

    dma_flag_clear(DMA1_FDT1_FLAG);
    printf("vbat_value = %f V\r\n", ((double)3.3 * 4 * adc1_ordinary_value) / 4095);
