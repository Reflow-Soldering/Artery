...
/** @addtogroup 437_QSPI_command_port_using_interrupt
  * @{
  */
...
volatile flag_status qspi_cmd_complete = SET;
...
void QSPI1_IRQHandler(void)
{
  if(qspi_interrupt_flag_get(QSPI1, QSPI_CMDSTS_FLAG) != RESET)
  {
    ...
    /* set cmd complete flag */
    qspi_cmd_complete = SET;
  }
}
...
void qspi_data_read(uint32_t addr, uint32_t total_len, uint8_t* buf)
{
  ...
  /* reset cmd complete flag */
  qspi_cmd_complete = RESET;
...
void qspi_busy_check(void)
{
  ...
  qspi_cmd_operation_kick(QSPI1, (qspi_cmd_type*)&en25qh128a_rdsr_para);
...
void qspi_write_enable(void)
{
  ...
  qspi_cmd_operation_kick(QSPI1, (qspi_cmd_type*)&en25qh128a_wren_para);
